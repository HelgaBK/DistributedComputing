...
import static client.OperationCodes.*;
...
public class Client {
    ...
    public static Client getInstance() throws JMSException {
        if(instance == null){
            instance = new Client();
        }
        return instance;
    }
    private Client() throws JMSException {
    ...
    public void deleteAircompany(int code) throws JMSException {
        TextMessage messageToServer = session.createTextMessage(String.valueOf(DELETE_AIRCOMPANY));
        producer.send(messageToServer);
    ...
    public void deleteFlight(int airCode, int code) throws JMSException {
        TextMessage messageToServer = session.createTextMessage(String.valueOf(DELETE_FLIGHT));
        producer.send(messageToServer);
